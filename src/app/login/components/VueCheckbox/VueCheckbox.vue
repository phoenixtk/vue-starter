<template>
  <div :class="cssClasses" :aria-label="label" ref="CheckboxDiv">
    <input
      :type="inputType"
      :name="name"
      :id="id"
  ...
  export default {
    ...
    computed: {
      cssClasses() {
        ...
        if (this.addClass) {
          classes.push(this.$style.loginCheckbox);
        }
    ...
    methods:  {
      onClick(e: Event) {
        ...
        if (this.disabled === false) {
          ...
          this.$emit('input', !Boolean(this.status));
        }
      },
    },
  };
...
<style lang="scss" module>
  ...
  .checkbox {
    ...
    .box {
      &:before, &:after {
        content:  "";
        position: absolute;
        left:     $checkbox-size / 8;
        top:      $checkbox-size / 5;
        cursor:   pointer;
      }

      &:before {
        width:      $checkbox-size - 0.4rem;
        height:     $checkbox-size - 0.4rem;
  ...
  .checkbox.loginCheckbox{
    $checkbox-color: $text-color-inverse;
    color:       $checkbox-color;
  }
