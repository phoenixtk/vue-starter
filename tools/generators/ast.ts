...
export const addModuleToStore = (pathToAppActions: string, moduleName: string, pathName: string): void => {
  try {
    ...
    file = insertAt(
      file,
      findAstNodes(sourceFile, ts.SyntaxKind.ImportDeclaration, true).pop().end,
      `\nimport { ${upperFirst(moduleName)}Module }            from './${pathName}${lowerFirst(moduleName)}/module';`,
    );
...
export const addModuleToState = (pathToAppActions: string, moduleName: string, pathName: string): void => {
  try {
    let file = fs.readFileSync(pathToAppActions, 'utf-8');
    ...
    file = insertAt(
      file,
      findAstNodes(sourceFile, ts.SyntaxKind.ImportDeclaration, true).pop().end,
      `\nimport { ${upperFirst(moduleName)}DefaultState, I${upperFirst(moduleName)}State }         from './${pathName}${lowerFirst(moduleName)}/state';`,
    );
...
export const addModuleToRoutesAframe = (pathToAppRoutes: string, moduleName: string, pathName: string): void => {
  try {
    let file = fs.readFileSync(pathToAppRoutes, 'utf-8');
    ...
    file = insertAt(
      ...
      `      {\n        path:      '${lowerFirst(moduleName)}',\n        component: () => import(/* webpackChunkName: "${lowerFirst(moduleName)}" */ '../${lowerFirst(pathName)}${lowerFirst(moduleName)}/${upperFirst(moduleName)}/${upperFirst(moduleName)}.vue').then((m: any) => m.default),\n      },\n`,
    );
